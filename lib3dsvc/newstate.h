...
class NewState {
public:
	virtual void Save(const void* ptr, std::size_t size, const char* name) = 0;
	virtual void Load(void* ptr, std::size_t size, const char* name) = 0;
...
public:
	NewStateDummy();
	ptrdiff_t GetLength() { return length; }
	void Rewind() { length = 0; }
	virtual void Save(const void* ptr, std::size_t size, const char* name);
	virtual void Load(void* ptr, std::size_t size, const char* name);
...
public:
	NewStateExternalBuffer(u8 *buffer, ptrdiff_t maxlength);
	ptrdiff_t GetLength() { return length; }
	void Rewind() { length = 0; }
...
private:
	u8 *const buffer;
	ptrdiff_t length;
	const ptrdiff_t maxlength;
};

struct FPtrs {
	void (*Save_)(const void* ptr, std::size_t size, const char* name);
	void (*Load_)(void* ptr, std::size_t size, const char* name);
